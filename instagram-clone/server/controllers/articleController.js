...
exports.feed = async (req, res, next) => {
  try {
    ...
    const skip = req.query.skip || 0; // 건널 뛸 도큐먼트의 수

    const articleCount = await Article.count(where); // 게시물 갯수
...
exports.find = async (req, res, next) => {
  try {
    ...
    const articles = await Article
      .find(where, 'photos favoriteCount created') // find(검색조건, 검색 필드)
      .populate({
    ...
    res.json({ articles, articleCount }); // 검색결과를 전송한다
...
exports.unfavorite = async (req, res, next) => {
  try {
    ...
    if (favorite) { // 좋아요 처리가 된 게시물이 맞으면
      await favorite.deleteOne(); // 도큐먼트 삭제

      article.favoriteCount--; // 게시물의 좋아요 수를 1 감소시킨다
      await article.save(); // 변경 사항 저장
    }
